...
void lcd_display(char* direction, char* speed) {
    lcd_init();  // Initialize the LCD display
    lcd_put_cur(0, 0);
    // Clear buffer and format the first line with direction and speed
    buff_clear(buffer);
    snprintf(buffer, 16, "Dir:%-4s %3sm/s", direction, speed);
    lcd_send_string(buffer);  // Display direction and speed on the first line
}

void lcd_display_time() {
    // Set cursor to the beginning of the second line
    lcd_put_cur(1, 0);
    // Clear buffer and format the elapsed time string
    buff_clear(buffer);
    snprintf(buffer, 16, "Time:%lu s   ", HAL_GetTick() / 1000);  // Convert milliseconds to seconds
    lcd_send_string(buffer);  // Display elapsed time on the second line
}

// Function to get distance from ultrasonic sensor in centimeters
float getDistance(GPIO_TypeDef* triggerPort, uint16_t triggerPin, GPIO_TypeDef* echoPort, uint16_t echoPin) {
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	if(GPIO_Pin == GPIO_PIN_0)
	{
		  ...
		  while (getDistance(GPIOC, GPIO_PIN_11,GPIOC, GPIO_PIN_12) < 15)
		  {  // While obstacle is too close
			leftDistance = getDistance(GPIOA, GPIO_PIN_11,GPIOA, GPIO_PIN_12);  // Update left distance
...
int main(void)
{
  ...
  {
	  lcd_display_time();
		leftDistance = getDistance(GPIOA, GPIO_PIN_11, GPIOA, GPIO_PIN_12);
		frontDistance = getDistance(GPIOC, GPIO_PIN_11, GPIOC, GPIO_PIN_12);
		HAL_Delay(100);
    	if(frontDistance <=FRONT_DESIRED_DISTANCE)
		{
			HAL_GPIO_WritePin(GPIOA, GPIO_PIN_0, GPIO_PIN_SET); //EXTI0
			HAL_Delay(100); // Ensure the interrupt is registered
			HAL_GPIO_WritePin(GPIOA, GPIO_PIN_0, GPIO_PIN_RESET); // Reset
		}
